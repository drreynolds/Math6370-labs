...
int linresid(double *a, double *b, double *c, double *u, double *r,
  ...
  *norm2 += res[k]*res[k];
  ...
  if ( MPI_Allreduce(norm2, &tmp, 1, MPI_DOUBLE, MPI_SUM, comm) != MPI_SUCCESS) {
    fprintf(stderr,"linresid error in MPI_Allreduce\n");
    return 1;
  }
